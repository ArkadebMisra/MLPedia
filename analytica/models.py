...
class KMeansCluster(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                            related_name='k_means_models_created',
                            on_delete=models.CASCADE)
    title = models.CharField(max_length=500)
    ...
    plot = models.ImageField(upload_to='images/%Y/%m/%d/', blank = True)
    weights_file = models.FileField(upload_to='files/%Y/%m/%d/', blank = True)
    k_means_output_file = models.FileField(upload_to='files/%Y/%m/%d/', blank = True)
    slug = models.SlugField(max_length = 600, blank=True)
    created = models.DateField(auto_now_add = True, db_index = True)
    ...
    def get_absolute_url(self):
        return reverse('analytica:k_means_detail', args=[self.id, self.slug])


class LogisticRegression(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                            related_name='logistic_regression_models_created',
                            on_delete=models.CASCADE)
    title = models.CharField(max_length=500)
    features_input = models.FileField(upload_to='files/%Y/%m/%d/')
    labels_input = models.FileField(upload_to='files/%Y/%m/%d/')
    ...
    def get_absolute_url(self):
        return reverse('analytica:logistic_regression_detail', args=[self.id, self.slug])



class Perceptron(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                            related_name='perceptron_models_created',
                            on_delete=models.CASCADE)
    title = models.CharField(max_length=500)
    features_input = models.FileField(upload_to='files/%Y/%m/%d/')
    labels_input = models.FileField(upload_to='files/%Y/%m/%d/')
    ...
    def get_absolute_url(self):
        return reverse('analytica:perceptron_detail', args=[self.id, self.slug])